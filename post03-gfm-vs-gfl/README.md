...
แหล่งจ่ายกริด 22 kV 50 Hz
   └ อิมพีแดนซ์กริด |Zg| = 484 Ω / SCR, X/R 10
      └ บัส 22 kV (SCR คิดที่บัสนี้ บนพิกัด 1 MVA)
         ├ โหลดตัวต้านทาน 500 kW ต่อที่เวลา 1 วินาที
         └ หม้อแปลง step up 1 MVA 22 kV/380 V อิมพีแดนซ์ 6% X/R 8
            └ อินเวอร์เตอร์ 1 MVA จ่าย 800 kW ตัวประกอบกำลัง 1
               ตัวกรองชุดเดียวกันทั้งสองแบบ Lf 45 µH Cf 1.33 mF Rf 0.75 mΩ Rc 0.75 mΩ
```

- **รูปที่ 1 ถึง 7 ในโพสต์สร้างจากไฟล์ `post03_dpsim.py` (หรือโน้ตบุ๊กตัวเดียวกัน) โดยตรง**
  สั่ง `python post03_dpsim.py <โฟลเดอร์>` จะได้ไฟล์ `fig1_system.png` ถึง `fig7_pros_cons.png` ชุดเดียวกับที่เห็นในโพสต์
  ตั้ง `QUICK = True` ถ้าอยากดูเร็ว (รันแผนที่แค่แถวเดียวต่อแบบ และใช้ขอบที่ไล่ไว้แล้ว)
- จำลองแบบ EMT สามเฟสด้วย DPsim 1.4.0 time step 50 µs
- โหลดที่บัส 22 kV ทำหน้าที่เป็นการกระตุ้นระบบ ต่อด้วยสวิตช์ที่ t = 1 s ไม่ได้อยู่ในจุดทำงานตั้งต้น
  ไล่ขนาดตั้งแต่ 50 kW ถึง 1 MW แล้ว สถานะของทุกจุดในแผนที่เหมือนเดิม เปลี่ยนแค่ขนาดการแกว่งซึ่งโตตามขนาดโหลด
